...
module.exports.top6ProductComboFollowIdCat = idCat => {
  return db.load(`SELECT pro_cb.ID, pro_cb.PRICE, pro_cb.SALE, pro_cb.NAME, pro_1.IMAGE AS IMAGE1, pro_2.IMAGE AS IMAGE2,
                ...
                ON pro_cb.CATEGORYID = ${idCat} AND pro_cb.PRODUCTID1 = pro_1.ID  AND pro_cb.PRODUCTID2 = pro_2.ID AND
...
// Hàm trả về 6 sản phẩm sắp xếp theo ngày đăng
module.exports.top6ProductComboAscCreated = () => {
  return db.load(`SELECT pro_cb.ID, pro_cb.PRICE, pro_cb.SALE, pro_cb.NAME, pro_1.IMAGE AS IMAGE1, pro_2.IMAGE AS IMAGE2,
                pro_3.IMAGE AS IMAGE3, pro_cb.CATEGORYID, pro_cb.SUBCATEGORYID,
                (CASE
                        WHEN pro_cb.SALE > 0 THEN (pro_cb.PRICE - pro_cb.PRICE * (pro_cb.SALE / 100))
